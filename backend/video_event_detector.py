...
class VideoEventDetector:
    ...
    def __init__(
        ...
        try:
            self.llm_provider: LLMProvider = LLMProviderFactory.create_provider(
                ...
                api_key=api_key,
                model=model,
                base_url=base_url if self.provider_name != "gemini" else None,
            )
    ...
    def detect_events(
        ...
        logger.info(
            f"Extracted {len(frames)} frames from {video_path}. Sending to {self.provider_name} LLM."
        )

        try:
            # Use the LLM provider abstraction
            results = self.llm_provider.analyze_frames(
                frames=frames,
                events=events,
                context=context,
            ...
            if "error" in results:
                logger.error(f"LLM provider returned error: {results['error']}")
                return results
